...
import { EditComponentsRequest, GetComponentsData, ScreenMessage } from '../entity'
import { getComponents } from '../service/api'

export function ComponentsEditScreen() {
  const [screenMessage, setScreenMessage] = useState<ScreenMessage | null>(null)
  const [ComponentsData, setComponentsData] = useState<GetComponentsData | null>(null)
  ...
  const getComponentsData = async () => {
    try {
      setLoading(true)
      const resData = await getComponents()
      ...
      if (resData.status === 'fail' && resData.message) {
        ...
        var ComponentsData: GetComponentsData = {
  ...
  const onSubmit = (data: EditComponentsRequest) => {
    console.log(data)
    var messageData: ScreenMessage = {
      error: false,
      message: 'Components berhasil disimpan',
    }
    setScreenMessage(messageData)
  }
  ...
  console.log("cek Components", ComponentsData)
        ...
        <View className="mt-10 p-5">
          ...
          <View>
            <Controller
              ...
              name="email"
              defaultValue={ComponentsData?.email}
            />
            {errors.email && (
            ...
            <Controller
              ...
              name="nama"
              defaultValue={ComponentsData?.name}
            />
            {errors.nama && (
              <Text className="mb-4 text-red-500">
            ...
            <Controller
              ...
              name="kota"
              defaultValue={ComponentsData?.city}
            />
            {errors.kota && (
              <Text className="mb-4 text-red-500">
